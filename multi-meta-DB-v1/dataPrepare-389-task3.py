...
class Data_triplets():
    ...
    def get_drug_related_triplets_dict(self):
        ...
        for tri in self.others_triplets:
            for tid in [0, 2]:
                i = tri[tid]
                if i in self.other_drug_dict_pool.keys():
                    self.other_drug_dict_pool[i].append(tri)
                else:
                    self.other_drug_dict_pool[i] = [tri]

def generate_task_data_train(drug, drug_sim, train_dict, test_dict,train_triple, test_triple, fea_mtx, training = True):  ##新加了传特征矩阵的参数
    if training:
        support_rate = 0.8
        meta_feat_mtx = []

        triplets = train_dict[drug]   #triplets: [[623, 3, 1711], [623, 2, 21], [623, 0, 345]]
        ...
        meta_feat_mtx = []
        triplets_train = train_dict[drug_sim]
...
class Meta(nn.Module):
    def __init__(self, num_inputs_in = 389):
...
#         self.update_lr= 0.01 #0.01
        ...
        # self.net = Mymodel_GAT_cov(num_inputs, num_outputs, num_hiddens, num_edg_hiddens)
#         self.net = Mymodel_GAT_shrink(num_inputs, num_outputs, num_hiddens, num_edg_hiddens)
    ...
    def finetune(self,drug_pool_batch_list_test,feat_mtrx_sel):
        ...
        feat_mtrx_sel_t = torch.tensor(feat_mtrx_sel).to(device)
        for drug in drug_pool_batch_list_test:
            drug_emb = feat_mtrx_sel_t[drug]
            pdist = nn.PairwiseDistance(p=2).to(device)
            dist = pdist(feat_mtrx_sel_t, drug_emb)
            train_id = list(train_drug_dict_pool.keys())
            dist = dist[train_id]
            ind = torch.topk(dist, 1, largest = False)[1]
            drug_sim = train_id[ind]
            feat_metrix_meta, spt_edg, spt_r, qry_edg, qry_r = generate_task_data_train(drug, drug_sim, train_drug_dict_pool, test_drug_dict_pool,train_set_triplets,test_set_triplets,feat_mtrx_sel, training = False)
            feat_metrix_meta, spt_edg, spt_r, qry_edg, qry_r =  feat_metrix_meta.to(device),spt_edg.to(device), spt_r.to(device), qry_edg.to(device), qry_r.to(device)
            logits = net(feat_metrix_meta, spt_edg)
#             loss = loss_func (logits, spt_r)
            loss = loss_func (logits, spt_r, weight=class_weights)
...
#                 loss_q = loss_func (logits_q, qry_r)
                loss_q = loss_func (logits_q, qry_r, weight=class_weights)
                with torch.no_grad():
        ...
        return accs_test, pred_score_list, y_truth_list
...
label = np.array(list(pd_ddi["type"]))
class_weights = np.ones(52)
...
class_weights[1] = 0.08
class_weights[2] = 0.16
class_weights[3] = 0.24
class_weights[4] = 0.32
class_weights[5] = 0.40
class_weights[6] = 0.48
class_weights[7] = 0.56
class_weights[8] = 0.64
class_weights[9] = 0.72
...
for idx in range(5):
    ...
    test_drug_dict_pool = data.other_drug_dict_pool
    train_set_triplets = data.train_set_triplets
    test_set_triplets = data.others_triplets
    feature_sel = [0, 1, 2, 3, 4, 5]
    maml = Meta(389).to(device)
    drug_pool_test= list(set(test_drug_dict_pool.keys()))
    print(len(list(set(train_drug_dict_pool.keys()))))
    print(len(drug_pool_test))
    pred_score_feature_best = []
    y_truth_feature_best = []
    for f in feature_sel:  
        maml.load_state_dict(torch.load("./ckpt/"+"389covgelu012345k50withpathwayadambatch1-"+str(idx)+"featurefusion-"+str(f)+"best.pt"))
        # maml.load_state_dict(torch.load("./ckpt/"+"389covcv012345k50withpathwayadambatch1-"+str(idx)+"featurefusion-"+str(f)+"best.pt"))
        if f == 0:
            feat_mtrx_sel = np.load("targetnew389.npy")
        if f == 1:
            feat_mtrx_sel = np.load("enzymenew389.npy")
        if f == 2:
            feat_mtrx_sel = np.load("pathwaynew389.npy")
        if f == 3:
        ...
        acc, pred_score_l_best, y_truth_l = maml.finetune(drug_pool_test, feat_mtrx_sel)
    ...
    if len(pred_score_feature_best) == 6:
        # pred_score_res = ((pred_score_feature_best[0] + pred_score_feature_best[1] + pred_score_feature_best[2] + pred_score_feature_best[3]+ pred_score_feature_best[4]+ pred_score_feature_best[5]) / 6).cpu().numpy()
        pred_score_res = ((pred_score_feature_best[0] + pred_score_feature_best[1]*0.6 + pred_score_feature_best[2] + pred_score_feature_best[3] + pred_score_feature_best[4]*0.6+ pred_score_feature_best[5]) / 5.2).cpu().numpy()
    ...
    print(cv_all_result)
    save_result(str(idx) + "task3389covgelu012345k50withpathwayadambatch1new", 'all', "META", cv_all_result)
    save_result(str(idx) + "task3389covgelu012345k50withpathwayadambatch1new", 'each', "META", cv_each_result)
    # save_result(str(idx) + "task3389covcv012345k50withpathwayadambatch1new", 'all', "META", cv_all_result)
    # save_result(str(idx) + "task3389covcv012345k50withpathwayadambatch1new", 'each', "META", cv_each_result)
...
save_result("task3389covgelu012345k50withpathwayadambatch1new", 'all', "META", all_result)
save_result("task3389covgelu012345k50withpathwayadambatch1new", 'each', "META", each_result)
# save_result("task3389covcv012345k50withpathwayadambatch1new", 'all', "META", all_result)
# save_result("task3389covcv012345k50withpathwayadambatch1new", 'each', "META", each_result)
