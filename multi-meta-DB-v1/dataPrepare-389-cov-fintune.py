...
def multiclass_precision_recall_curve(y_true, y_score):
    ...
    y_score_c = y_score.take([0], axis=1).ravel()
    precision, recall, pr_thresholds = precision_recall_curve(y_true_c, y_score_c)
    fpr, tpr, roc_thresholds= roc_curve(y_true_c, y_score_c)
    return (precision, recall, pr_thresholds, fpr, tpr, roc_thresholds)
...
def evaluate(pred_type, pred_score, y_test, event_num, set_name):
    ...
    y_one_hot = label_binarize(y_test, classes=np.arange(event_num))
    pred_one_hot = label_binarize(pred_type, classes=np.arange(event_num))
    precision, recall, th, fpr, tpr, roc_thresholds = multiclass_precision_recall_curve(y_one_hot, pred_score)
    np.save("GAMT"+"precision", precision)
    np.save("GAMT"+"recall", recall)
    np.save("GAMT"+"prth", th)
    np.save("GAMT"+"fpr", fpr)
    np.save("GAMT"+"tpr", tpr)
    np.save("GAMT"+"rocth", roc_thresholds)
    result_all[0] = accuracy_score(y_test, pred_type)
    result_all[1] = roc_aupr_score(y_one_hot, pred_score, average='micro')
...
for idx in [0, 1, 2, 3, 4]:
    ...
    for f in feature_sel:  
        ...
        if f == 1230:
            feat_mtrx_sel = np.concatenate((np.load("targetover40.npy"), np.load("enzymeover40.npy"), np.load("pathwayover40.npy"), np.load("smileover40.npy")), axis=1)
    ...
    print(cv_all_result)
    save_result(str(idx) + "task2389covgelu012345k50withpathwayadambatch1new1", 'all', "META", cv_all_result)
    save_result(str(idx) + "task2389covgelu012345k50withpathwayadambatch1new1", 'each', "META", cv_each_result)
    #save_result(str(idx) + "task2389covcv012345k50withpathwayadambatch1new", 'all', "META", cv_all_result)
    #save_result(str(idx) + "task2389covcv012345k50withpathwayadambatch1new", 'each', "META", cv_each_result)
...
save_result("task2389covgelu012345k50withpathwayadambatch1new1", 'all', "META", all_result)
save_result("task2389covgelu012345k50withpathwayadambatch1new1", 'each', "META", each_result)
#save_result("task2389covcv012345k50withpathwayadambatch1new", 'all', "META", all_result)
#save_result("task2389covcv012345k50withpathwayadambatch1new", 'each', "META", each_result)
