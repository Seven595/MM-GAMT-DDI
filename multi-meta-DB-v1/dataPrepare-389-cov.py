...
for idx in [0, 1, 2, 3, 4]:
    ...
    for f in feature_sel:  
        ...
        if f == 1230:
            ...
            print(feat_mtrx_sel.shape)
        epochs = 250
        best_acc = -1
        count = 0
        best_loss = 1000000.
        for epoch in range(epochs):
            for i, batch in enumerate(data_iter(batcsz, drug_pool_train)):
                acc = maml(batch, feat_mtrx_sel)
            acc, pred_score_l, y_truth_l, l = maml.finetune(drug_pool_test.tolist(), feat_mtrx_sel)
            if(acc > best_acc):
                best_acc = acc
                pred_score_l_best = deepcopy(pred_score_l)
                torch.save(maml.state_dict(), "./ckpt/"+"389covgelu012345k50withpathwayadambatch1-"+str(idx)+"featurefusion-"+str(f)+"best.pt") 
                count = 0
            else:
                count+=1
            if count > 100:
                break
            print_file("389covgelu012345k50withpathwayadambatch1 epoch: {}, acc:{}, loss: {}".format(epoch, acc, l), "./result.out")
    ...
    if len(pred_score_feature_best) == 4:
        ...
        # pred_score_res =  pred_score_feature_best[3].cpu().numpy()
        # pred_score_res = (pred_score_feature_best[0]*0.25 + pred_score_feature_best[1]*0.35 + pred_score_feature_best[2]*0.25 + pred_score_feature_best[3]*0.15).cpu().numpy()
    ...
    print(cv_all_result)
    save_result(str(idx) + "task2389covgelu012345k50withpathwayadambatch1", 'all', "META", cv_all_result)
    save_result(str(idx) + "task2389covgelu012345k50withpathwayadambatch1", 'each', "META", cv_each_result)
...
save_result("task2389covgelu012345k50withpathwayadambatch1", 'all', "META", all_result)
save_result("task2389covgelu012345k50withpathwayadambatch1", 'each', "META", each_result)
